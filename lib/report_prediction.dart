...
String url = "b308-103-248-123-92.ngrok-free.app";
                ...
                children: [
                  Row(
                    ...
                    children: [
                      const Icon(Icons.analytics_outlined,
                        color: Colors.black54,
                        size: 24,),
                      const Text("Blood Report Analysis",
                        style: TextStyle(
                          fontSize: 14,
                          fontFamily: 'Poppins',
                          fontWeight: FontWeight.bold,
                  ...
                  Container(
                      ...
                      child: InkWell(
                        ...
                        onTap: () {
                          // print('skin prediction');
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>ReportAnalysis(number: number)));
                        },
                      )
                  ),
                ],
...
class _SkinClassifierState extends State<SkinClassifier> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFFFFFFF),
      appBar: AppBar(
        title: const Text('Folliculitis Classifier',style: TextStyle(color: Colors.black54, fontFamily: 'Poppins', fontWeight: FontWeight.bold)),
        backgroundColor: const Color(0xFFFFFFFF),
        elevation: 0,
        leading: ElevatedButton(
          onPressed: () {
...
class ReportAnalysis extends StatefulWidget {
  final String number;
  const ReportAnalysis({Key? key, required this.number}) : super(key: key);
...
class _ReportAnalysisState extends State<ReportAnalysis> {
  ...
  String pdf = "", display = "         Choose PDF";
  var uuid;

  Future<String> getPdfLink() async {
    File file = File(pdf);
    Reference refer = FirebaseStorage.instance.ref();
    Reference ref_dir = refer.child('prediction_pdf');
    Reference upload = ref_dir.child(DateTime.now().toIso8601String());
    await upload.putFile(file);
    String link = await upload.getDownloadURL();
    return link;
  ...
  Future<String> fetch() async {
    String getLink = await getPdfLink();
    final queryParams = {
      'type': "3",
      'filename': uuid.v4().toString(),
      'pdf_file': getLink
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Blood Report Analysis',style: TextStyle(color: Colors.black54, fontFamily: 'Poppins', fontWeight: FontWeight.bold)),
      ),
      body: Column(
        // mainAxisAlignment: MainAxisAlignment.center,
        children: [
              ...
              child: Container(
                ...
                child:ElevatedButton(onPressed: () async {
                  ...
                  if(result==null) {return;}
                    ...
                    pdf = result.files.first.path.toString();
                    String media = path.basename(pdf);
                    display = "         ";
                    int i = 0;
                    for(i=0;i<media.length && i<14;i++) {
                      display+=media[i];
